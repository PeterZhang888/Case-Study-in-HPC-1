/*
  casestudy1.c : Standard (non-optimised) TSQR for 4 MPI ranks.

  - Each rank owns m/4 rows of A (A_local is (mloc x n), row-major).
  - Uses LAPACKE_dgeqrf + LAPACKE_dorgqr for local QR.
  - Reduction tree: (0,1)->0 ; (2,3)->2 ; then (0,2)->0.
  - Produces:
...
void TSQR(const double *W_i, int mloc, int n,double *Q_i_final, double *R_root_out,MPI_Comm comm){
    ...
    // 2) Level-1 reduction: (0,1)->0 and (2,3)->2
    double *R_pair = NULL;
    int partner=-1;
    if (rank==0) partner=1;
    if (rank==2) partner=3;
...
int main(int argc, char **argv)
{
    ...
    // Choose a tall-skinny size; ensure m divisible by 4
    const int m = 400;
    const int n = 10;
    const int mloc = m / 4;
    ...
    // Fill W_i with deterministic values (so you can reproduce results)
